...
def preprocess_data_multiprocessing(data):
    if len(data) == 4:
        ...
        text_a, text_b, text_a_pos, text_b_pos, text_a_neg, text_b_neg, tokenizer, max_seq_length = data
    
        inputs_org =  tokenizer(
            text=text_a,
            text_pair=text_b,
            truncation=True,
            padding="max_length",
        ...
        final_inputs = {}
        for key in inputs_org.keys():
            final_inputs[key + '_org'] = inputs_org[key]
            final_inputs[key + '_pos'] = inputs_pos[key]
            final_inputs[key + "_neg"] = inputs_neg[key]
...
def preprocess_data_cts(text_a, text_b, text_a_pos, text_b_pos, text_a_neg, text_b_neg, labels, tokenizer, max_seq_length):
    inputs_org =  tokenizer(
        text=text_a,
        text_pair=text_b,
        truncation=True,
...
def build_classification_dataset(
    ...
    if os.path.exists(cached_features_file) and (
        ...
        if len(data) == 3:
            ...
            text_a, text_b, text_a_pos, text_b_pos, text_a_neg, text_b_neg, labels = data
        else:
            text_a, labels = data
            text_b = None
        ...
        if (mode == "train" and args.use_multiprocessing) or (
            ...
            if text_b is not None and text_a_pos is None:
                data = [
                    (
            ...
            elif text_b is not None and text_a_pos is not None:
                data = [
                    (
                        text_a[i : i + chunksize],
                        text_b[i : i + chunksize],
                        text_a_pos[i : i + chunksize],
                        text_b_pos[i : i + chunksize],
                        text_a_neg[i : i + chunksize],
                        text_b_neg[i : i + chunksize],
                        tokenizer,
                        args.max_seq_length,
                    )
                    for i in range(0, len(text_a), chunksize)
            ...
            if text_a_pos is None:
                examples = preprocess_data(
                    text_a, text_b, labels, tokenizer, args.max_seq_length
                )
            else:
                examples = preprocess_data_cts(
                    text_a, text_b, text_a_pos, text_b_pos, text_a_neg, text_b_neg, labels, tokenizer, args.max_seq_length
                )
...
class ClassificationDataset(Dataset):
    ...
    def __len__(self):
        if "input_ids" in self.examples.keys():
            return len(self.examples["input_ids"])
        else:
            return len(self.examples["input_ids_org"])
