...
from .classification_utils import ClassificationDataset
...
class ClassificationModel:
    def __init__(
        ...
        **kwargs,
        ...
        self.cts = cts
        self.cl = cl
        print("Loading Custom ClassificationModel...")
    ...
    def train_model(
        ...
        **kwargs,
        ...
        if self.args.use_hf_datasets:
            ...
            elif "text_a" in train_df.columns and "text_b" in train_df.columns and "text_a_pos" not in train_df.columns:
                if self.args.model_type == "layoutlm":
            ...
            elif "text_a" in train_df.columns and "text_b" in train_df.columns and "text_a_pos" in train_df.columns and "text_b_pos" in train_df.columns and "text_a_neg" in train_df.columns and "text_b_neg" in train_df.columns:
                if self.args.model_type == "layoutlm":
                    raise ValueError("LayoutLM cannot be used with sentence-pair tasks")
                else:
                    train_examples = (
                        train_df["text_a"].astype(str).tolist(),
                        train_df["text_b"].astype(str).tolist(),
                        train_df["text_a_pos"].astype(str).tolist(),
                        train_df["text_b_pos"].astype(str).tolist(),
                        train_df["text_a_neg"].astype(str).tolist(),
                        train_df["text_b_neg"].astype(str).tolist(),
                        train_df["labels"].tolist(),
                    )
            else:
                warnings.warn(
    ...
    def train(
        ...
        **kwargs,
        ...
        cl_indices = [i for i in range(self.train_dataset.__len__())]
        ...
        if self.cl == 'vanilla':
            ...
            difficulties = self.evaluate_difficulty_cts(difficulty_df)
            print(difficulties)
            difficulties = [[i,d] for i, d in enumerate(difficulties)]
        ...
        for _ in train_iterator:
            ...
            if self.cl == 'vanilla':
                train_dataset_ordered = Subset(self.train_dataset, indices=cl_indices)
                train_dataloader = DataLoader(
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                inputs, inputs_pos, inputs_neg = self._get_inputs_dict_cts(batch)
                ...
                if self.args.fp16:
                    ...
                    loss = outputs[0]
                    if self.cts == "True":
                        org_r = torch.zeros((outputs[2][-1].shape[0], outputs[2][-1].shape[2]))
                        pos_r = torch.zeros((outputs[2][-1].shape[0], outputs[2][-1].shape[2]))
                        neg_r = torch.zeros((outputs[2][-1].shape[0], outputs[2][-1].shape[2]))
                        for i in range(inputs['input_ids'].shape[0]):
                            target_index_org = (inputs['input_ids'][i] == 2).nonzero(as_tuple=True)[0]
                            target_index_pos = (inputs_pos['input_ids'][i] == 2).nonzero(as_tuple=True)[0]
                            target_index_neg = (inputs_neg['input_ids'][i] == 2).nonzero(as_tuple=True)[0]
                            org_ = torch.mean(outputs[2][-1][i, target_index_org[1]+1:target_index_org[2], :], dim = 0)
                            pos_ = torch.mean(outputs_pos[2][-1][i, target_index_pos[1]+1:target_index_pos[2], :], dim = 0)
                            neg_ = torch.mean(outputs_neg[2][-1][i, target_index_neg[1]+1:target_index_neg[2], :], dim = 0)
                            org_r[i, :] = org_
                            pos_r[i, :] = pos_
                            neg_r[i, :] = neg_
                        pos_cos = self.cos(org_r, pos_r).reshape(outputs[2][-1].shape[0], 1)
                        neg_cos = self.cos(org_r, neg_r).reshape(outputs[2][-1].shape[0], 1)
    ...
    def eval_model_difficulty(
        ...
        **kwargs,
        ...
        result_difficulty = self.evaluate_difficulty(
            eval_df,
            output_dir,
            multi_label=multi_label,
            verbose=verbose,
    ...
    def evaluate_difficulty(
        ...
        **kwargs,
        ...
        n_batches = len(eval_dataloader)
        difficulties = 0.0
        literal_r = torch.zeros((768))
        figurative_r = torch.zeros((768))
        literal_r_list = []
        figurative_r_list = []
        literal_num = 0.0
        figurative_num = 0.0
        ...
        for i, batch in enumerate(
            ...
            with torch.no_grad():
                ...
                if self.args.fp16:
                    ...
                    outputs = model(**inputs)
                    tmp_eval_loss, logits = outputs[:2]
                    context_representations = outputs[2][-1]
                    word_embeddings = outputs[2][0]
                    for i in range(context_representations.shape[0]):
                        target_index = (inputs['input_ids'][i] == 2).nonzero(as_tuple=True)[0]
                        ctx_r = torch.mean(context_representations[i, target_index[1]+1:target_index[2], :], 0)
                        emb_r = torch.mean(word_embeddings[i, target_index[1]+1:target_index[2], :], 0)
                        if inputs['labels'][i] == 1:
                            #difficulty = 1 - cos(ctx_r, emb_r)
                            literal_num += 1
                            literal_r += ctx_r.cpu().numpy()
                            literal_r_list.append(ctx_r.cpu().numpy())
                        else:
                            #difficulty = cos(ctx_r, emb_r)
                            figurative_num += 1
                            figurative_r += ctx_r.cpu().numpy()
                            figurative_r_list.append(ctx_r.cpu().numpy())
        ...
        literal_r = literal_r / literal_num
        figurative_r = figurative_r / figurative_num
        difficulties = cos(literal_r, figurative_r)

        return difficulties, literal_r, figurative_r, literal_r_list, figurative_r_list
    
    def evaluate_difficulty_cts(
        self,
        eval_df,
        multi_label=False,
        prefix="",
        ...
        **kwargs,
        ...
        if self.args.use_hf_datasets:
            ...
            elif "text_a" in eval_df.columns and "text_b" in eval_df.columns and "text_a_pos" not in eval_df.columns:
                if self.args.model_type == "layoutlm":
                    raise ValueError("LayoutLM cannot be used with sentence-pair tasks")
                else:
                    eval_examples = (
            ...
            elif "text_a" in eval_df.columns and "text_a_pos" in eval_df.columns:
                if self.args.model_type == "layoutlm":
                    raise ValueError("LayoutLM cannot be used with sentence-pair tasks")
                else:
                    eval_examples = (
                        eval_df["text_a"].astype(str).tolist(),
                        eval_df["text_b"].astype(str).tolist(),
                        eval_df["text_a_pos"].astype(str).tolist(),
                        eval_df["text_b_pos"].astype(str).tolist(),
                        eval_df["text_a_neg"].astype(str).tolist(),
                        eval_df["text_b_neg"].astype(str).tolist(),
                        eval_df["labels"].tolist(),
                    )
            else:
                warnings.warn(
    ...
    def _get_inputs_dict(self, batch, no_hf=False):
        ...
        if isinstance(batch[0], dict):
            ...
            inputs_org["labels"] = batch[1].to(self.device)
        else:
            batch = tuple(t.to(self.device) for t in batch)
        ...
        if self.args.model_type == "layoutlm":
            inputs_org["bbox"] = batch[4]

        return inputs_org
    
    def _get_inputs_dict_cts(self, batch, no_hf=False):
        if self.args.use_hf_datasets and not no_hf:
            return {key: value.to(self.device) for key, value in batch.items()}
        if isinstance(batch[0], dict):
            inputs = {
            ...
            inputs_org = {
                "input_ids": inputs['input_ids_org'],
                "attention_mask": inputs['attention_mask_org'],
        ...
        return inputs_org, inputs_pos, inputs_neg
